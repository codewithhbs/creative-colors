...
const Products = () => {
  ...
  useEffect(() => {
    axios.get('https://www.api.creativencolourful.com/api/v1/get-product')
      .then(response => {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      const res = await axios.delete(`https://www.api.creativencolourful.com/api/v1/delete-product/${id}`)
      if (res.data.success) {
  ...
  const handleToggleShowOnHome = async (productId, currentValue) => {
    try {
      const response = await axios.put(`https://www.api.creativencolourful.com/api/v1/update-show-home/${productId}`, {
        isShowOnHomeScreen: !currentValue,
      });
        ...
        setProducts(prev =>
          prev.map(product =>
            product._id === productId
              ? { ...product, isShowOnHomeScreen: !currentValue }
              : product
          )
        );
        toast.success('Display on home updated!');
      } else {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentProducts.map((product) => (
              <tr key={product._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                    onClick={() => handleToggleShowOnHome(product._id, product.isShowOnHomeScreen)}
                    className={`px-3 py-1 text-xs font-semibold rounded-full ${product.isShowOnHomeScreen
                      ? 'bg-green-100 text-green-800'
                      : 'bg-gray-100 text-gray-800'
                      }`}
                  >
                    {product.isShowOnHomeScreen ? 'Shown' : 'Hidden'}
                  </button>
                </td>
